...
from tensorflow.keras.layers import GRU, LSTM, Bidirectional, Dense, Dropout, Embedding
...
def transform_and_pad(
    processed_texts, tokenizer, max_sequence_length, dataset_name="Data"
):
    """
...
def hyperparameter_search(
    ...
    param_grid,
    max_models=None,
    results_filename="hp_results.json",
):
    """
    ...
    for i, params in enumerate(combos):
        ...
        plot_history(
            history,
            title=f"{kind.upper()}_hu{params['hidden_units']}_dr{params['dropout_rate']}_te{int(params['trainable_embeddings'])}_lr{params['lr']}",
        )
...
def evaluate_on_validation(model, X_val, y_val, label_prefix, class_names=None):
    ...
    report = classification_report(
        y_val, preds, target_names=class_names, zero_division=0
    )
    print(report)
...
def evaluate_best_hp_and_save(hp_summary, X_val, y_val, class_names, label_prefix):
    ...
    metrics = evaluate_on_validation(
        best_model, X_val, y_val, label_prefix, class_names=class_names
    )
...
def predict_test_and_save(model, tokenizer, max_len=66):
    ...
    if not os.path.exists(TEST_FILE):
        ...
        test_sequences = transform_and_pad(
            test_processed, tokenizer, MAX_SEQUENCE_LENGTH, "Test Data"
        )
        ...
        ):  # maybe we got the true labels of test and we can compare to predictions
...
if __name__ == "__main__":
    ...
    hp_summary_lstm = hyperparameter_search(
        "lstm",
        X_train,
        y_train,
        X_val,
    ...
    hp_summary_gru = hyperparameter_search(
        "gru",
        X_train,
        y_train,
        X_val,
    ...
    hp_best_metrics_lstm = evaluate_best_hp_and_save(
        hp_summary_lstm, X_val, y_val, class_names, "lstm_hp"
    )
    hp_best_metrics_gru = evaluate_best_hp_and_save(
        hp_summary_gru, X_val, y_val, class_names, "gru_hp"
    )
    ...
    if selected_model_path is not None:
        ...
        print(
            "No model found for test predictions. Place a trained model in ./models or run HP search first."
        )
